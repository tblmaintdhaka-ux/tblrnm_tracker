...
FOLDER_ID = "1T18SJbmuGT1wf8W_lY1zXTgNjR4hRY73"
...
def download_db_from_drive():
    """Downloads the SQLite DB from Drive to the local ephemeral container."""
...
def upload_db_to_drive():
    """Uploads/Updates the local SQLite DB back to Google Drive."""
    service = get_drive_service()
    query = f"name = '{DB_FILE}' and '{FOLDER_ID}' in parents and trashed = false"
    results = service.files().list(q=query, fields="files(id)").execute()
    files = results.get('files', [])
...
if 'db_synced' not in st.session_state:
    if download_db_from_drive():
        st.toast("Database synced from Google Drive ✅")
    else:
        st.warning("No remote database found. Initializing fresh DB.")
    st.session_state['db_synced'] = True

# --- DATABASE & SECURITY FUNCTIONS ---
def execute_query(query, params=()):
    """Executes SQL and triggers an immediate Cloud Sync."""
...
def message_board():
    # Use columns to place the title and a help button side-by-side
    ...
    with col_help:
        # Help Button using a Popover to show the attached documentation
        with st.popover("📖 Help Guide"):
            st.markdown("### 🚀 Quick Start Guide: TBL R&M Tracker 2026")
            st.info("আপনার মেইনটেন্যান্স রিকোয়েস্ট এবং বাজেট পরিচালনা করতে নিচের ধাপগুলো অনুসরণ করুন।")
            
            st.markdown("""
            **১. লগ-ইন করা (Login)**
            - আপনার ইউজারনেম এবং পাসওয়ার্ড দিয়ে প্রবেশ করুন।
            
            **২. নতুন মেইনটেন্যান্স রিকোয়েস্ট (New Request)**
            - সঠিক **Cost Area** এবং কারেন্সি সিলেক্ট করুন। 
            - বিদেশী মুদ্রার ক্ষেত্রে সিস্টেম ৫% কাস্টমস ডিউটি যোগ করবে।
            
            **৩. বাজেট ট্র্যাকিং (Dashboard)**
            - 🟢 সবুজ: স্বাভাবিক খরচ।
            - 🟡 হলুদ: সতর্কীকরণ।
            - 🔴 লাল: বাজেট শেষ।
            
            **৪. পারচেজ এন্ট্রি (Purchase Entry)**
            - অনুমোদিত MN এর বিপরীতে বিলের তথ্য এবং আইটেম ডিটেইলস এন্ট্রি দিন।
            """)
            st.caption("বিস্তারিত সহায়তার জন্য অ্যাডমিনের সাথে যোগাযোগ করুন।")
